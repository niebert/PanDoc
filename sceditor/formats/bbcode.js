...
(function (sceditor) {
    ...
    header: {
      // menu option 'header'
        ...
        getEditorCommand('header')._dropDown(
					...
					caller,
					function (headerSize) {
            console.log("sceditor/formats/bbcode.js:113 - insert header with size="+headerSize);
						editor.insertText(
							'[header=' + (7-headerSize) + ']',
							'[/header]'
						);
  ...
  // the bbcodeHandlers are used
  // (1) to create the source code, when toggled to source code is clicked
  // (2) the bbcode handler defines how the bbcode tags are transformed into HTML
	...
	function BBCodeParser(options) {
		...
		base.tokenize = function (str) {
			...
			while (str.length) {
				i = tokenTypes.length;
				while (i--) {
          //console.log("sceditor/formats/bbcode.js:1125 -");
          ...
          console.log("sceditor/formats/bbcode.js:1125 - str='"+str+"' type["+i+"]='"+type+"' tokenType["+i+"]='"+JSON.stringify(tokenTypes[i],null,4)+"'");
        ...
        console.log("sceditor/formats/bbcode.js:1174 - name='"+name+"' matched with openRegex");
				if (matches[2] && (matches[2] = matches[2].trim())) {
					attrs = tokenizeAttrs(matches[2]);
				}
			}// else {
        //console.log("val='"+val+"' does not match openRegex");
      //}
      ...
      var tokenlist = base.tokenize(str);
      console.log("sceditor/formats/bbcode.js:1266 - str='"+str+"' tokenlist="+JSON.stringify(tokenlist,null,4));
			var ret  = parseTokens(tokenlist);
			var opts = base.opts;
     ...
     */
    base.toTREE = function (str, preserveNewLines) {
      return base.parse(str, preserveNewLines);
    };
      ...
      // is different from toHtml() method
      console.log("sceditor/formats/bbcode.js:1832 - CALL: toHTML('bbcode')");
      var tokenlist = base.parse(str, preserveNewLines);
      console.log("sceditor/formats/bbcode:1835 - tokenlist="+JSON.stringify(tokenlist,null,4));
      return convertToHTML(tokenlist, true);
		};
      ...
      console.log("sceditor/formats/bbcode.js:1999 - str='"+str+"'");
		...
		function convertToBBCode(toks) {
			...
			while (toks.length > 0) {
				...
				// TODO: tidy this
        console.log("sceditor/formats/bbcode.js:2022 - token.name='"+token.name+"' token.type='"+token.type+"'");
				bbcode        = bbcodeHandlers[token.name];
        console.log("sceditor/formats/bbcode.js:2024 - token handler for '"+token.name+"' bbcodeHandlers['"+token.name+"']="+JSON.stringify(bbcode,null,4));
	...
	function bbcodeFormat() {
      ...
      // in this method HTML header tags are removed
			var toBBCode = function (node, vChildren) {
				...
				dom.traverse(node, function (node) {
					var	content      = '',
						...
						isValidChild = true;
          console.log("sceditor/formats/bbcode.js:2539 - traverse DOM tag='"+tag+"'");
      ...
      console.log("sceditor/sceditor.js:2577 - CALL: toHtml('bbcode')");
      ...
      //console.log("sceditor/formats/bbcode.js:2646 - toSource(html) html='"+html+"'");
      ...
      console.log("sceditor/formats/bbcode.js:2682 - toSource(html) container.innerHTML='"+container.innerHTML+"'");
      // in elementToBbcode() HTML header tags are deleted
      bbcode = elementToBbcode(container);
      console.log("sceditor/formats/bbcode.js:2686 - bbcode='"+bbcode+"'");
  ...
  bbcodeFormat["name4format"] = "bbcode";
