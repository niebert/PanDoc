...
(function (sceditor) {
    ...
    header: {
			txtExec: function (caller) {
				var editor = this;
        console.log("sceditor/formats/xhtml.js:84");

				getEditorCommand('header')._dropDown(
					editor,
					caller,
					function (headerSize) {
            //alert("insert Header H"+headerSize);
						editor.insertText(
							'<h' + headerSize + '>',
							'</h' + headerSize + '>'
						);
    ...
    size: {
			txtExec: function (caller) {
        console.log("sceditor/formats/xhtml.js:114");
        var editor = this;
