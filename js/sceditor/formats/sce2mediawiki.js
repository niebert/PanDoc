...
// ---------------------------------------
// --- Version:   3.2.15
// --- Date/Time: 2025/12/06 12:37:58
// ---------------------------------------
/*! SCE2MediaWiki  Parser Plugin
 * Based on SCEditor by Sam Clarke - https://github.com/samclarke/SCEditor
 * Released under the MIT license
 *
 * Created: 2016/12/28
 * Build:   2025/12/06 12:37:58
 * Repository of current build: https://github.com/niebert/sce2mediawiki
 *
  ...
  */
  ...
  function get_default_doc(pLanguage,pDomain) {
    ...
    return {
      ...
      "timeid": Date.now(),
      "date": now.toJSON(),
      "mediapath":"https:/" + "/"+pLanguage+".wikiversity.org/wiki/Special:Redirect/file/",
...
//---- END - mediawiki01_wordmaps.js ----------------
//---- BEGIN -- mediawiki02_converter.js -------------------
function convertLínk2HTML(token, attrs, content) {
  ...
  var link = '<a href="' +
    ...
    '">' + content + '</a>';
    if (attrs["linktype"]) {
      console.log("sceditor/formats/src/mediawiki02_converter.js:8 - LINK_TYPE: TOKEN['"+attrs["linktype"]+"']")
...
function linked_text(content) {
  ...
  console.log("sceditor/formats/src/mediawiki02_converter.js:59 - linked_text('"+content+"')='"+ret+"'")
  return ret;
}
...
//---- END -- mediawiki02_converter.js -------------------
// ---- BEGIN -- mediawiki03_defaultcmds.js -------------------
...
//---- END -- mediawiki03_defaultcmds.js -------------------
//---- BEGIN -- mediawiki04_mediawikihandlers.js ---------
...
|-
  ...
  url: {
    ...
    format: function (element, content) {
      ...
      var url = attr(element, 'href4link') || "unknown-url";
      var link = attr(element, 'link') || "";
...
//---- END -- mediawiki04_mediawikihandlers.js ---------
...
function MediaWikiParser(options) {
  ...
  */
  ...
  function isChildAllowed(parent, child) {
    if (parent) {
      ...
      console.log("isChildAllowed(parent,child) - parent is not defined - so child is root child");
      return true;
    }
  }
  ...
  function isChildAllowed4TagName(parent_name, child_name) {
    ...
    if (allowedChildren && allowedChildren.indexOf("*") >= 0) {
      ...
      if ((child_name == "#") || mediawikiHandlers[child_name]) {
        console.log("sceditor/formats/mediawiki.js:2871 - isChildAllowed4TagName(parent='"+parent_name+"',child='"+child_name+"') ALL Children allowed");
        return true;
      }
    }
    if (base.opts.fixInvalidChildren && allowedChildren) {
      //alert("sceditor/formats/mediawiki.js:2784 - isChildAllowed(parent='"+parent_name+"',child='"+child_name+"')");
      console.log("sceditor/formats/mediawiki.js:2784 - isChildAllowed4TagName(parent='"+parent_name+"',child='"+child_name+"')");
  ...
  function tokenizeMath4Source(str,doc) {
    console.log("sceditor/formats/mediawiki.js:3054 - tokenizeMath4Source()");
    //alert("sceditor/formats/mediawiki.js:3054 - NOTHING-DONE - tokenizeMath4Source(str,doc)")
    return str;
  }

  function tokenizeMath(str,doc,toFormat) {
    console.log("sceditor/formats/mediawiki.js:3065 - tokenizeMath()");
  ...
  function detokenizeMath(str,doc,toFormat) {
    console.log("sceditor/formats/mediawiki.js:3075 - postProcessTable()");
  ...
  */
  ...
  function tokenizeMath4HTML(wiki, data, pOptions) {
    ...
    wiki = wiki.replace(/(\n[:]+)[\s]*<math([^>]*?)>([\s\S]+?)<\/math>/g, function (pMatch, prefix, attrs, inside) {
      vCount++;
      display = "block";
      if (attrs && attrs.indexOf("display")) {
  ...
  */
  ...
  base.toTREE = function (str, preserveNewLines, doc) {
    console.log("sceditor/formats/mediawiki.js:2905 - CALL: toTREE('mediawiki')");
 ...
 * @param {boolean} [legacyAsFragment] Used by fromMediaWikiCode() method
 */
function toTree(asFragment, source, legacyAsFragment) {
  ...
  if (data.hasOwnProperty("mathexpr")) {
    for (var i = 0; i < data.mathexpr.length; i++) {
      ...
      if (detok.type=="block") {
        ...
        text = text.replace(detok.label,"<math display=\"block\">"+detok.math+"</math>");
      } else if (detok.type=="inline") {
        //text = replaceString(text,detok.label,"<math display=\"inline\" id=\""+get_unique_id("MATH")+"\">\n  "+detok.math+"\n</math>");
        //text = text.replace(detok.label,"<math display=\"inline\" id=\""+get_unique_id("MATH")+"\">"+detok.math+"</math>");
        text = text.replace(detok.label,"<math display=\"inline\">"+detok.math+"</math>");
      }
    }
  }
  return text
